...
function gameEnd() {
    results.sendResultToServer(movesCount, seconds, minutes)
    .then(() => {
        showTopPlayers()
...
function renderAndStartGame() {
    ...
    u.getImagesByCategorie(imageType)
        .then(data => {
            playeble = true;
...
function renderIndividualRecord (localId) {
    results.getUserRecord(localId)
    .then(response => {
        document.getElementById('individual-record-contain').innerHTML = results.userRecordSection(response)
        document.getElementById('username').addEventListener('click' , ()=> {
            results.getUserIndividualResults(u.getLocalId())
            .then(response => {
...
function renderSignInForm() {
    ...
    signInForm.addEventListener('submit', (e) => {
        ...
        auth.signInByEmailAndPassword(signInForm.email.value, signInForm.password.value)
        .then(response => {
            signResponseHandling(response)
            return response;
        })
        .then(() => {
            auth.getUserByLocalId(u.getLocalId())
            .then(response => {
...
function renderSignUpForm() {
    ...
    signUpForm.addEventListener('submit', (e) => {
        ...
        auth.signUpByEmailAndPassword(signUpForm.email.value, signUpForm.password.value)
        .then(response => {
            signResponseHandling(response)
            return response
        })
        .then(response => {
            ...
            auth.createUser(signUpForm.username.value, response.email, response.localId)
            .then(() => {
                auth.getUserByLocalId(u.getLocalId())
                .then(response => {
                    u.usernameToLocalStorage(response.username)
...
function showTopPlayers() {
    results.getTopPlayers()
    .then(response => {
        document.getElementById('top-players-list').innerHTML = results.renderTopPlayersComponent(response)
    })
}
